...
export async function POST(request: NextRequest) {
  try {
    ...
    const { timeHorizon } = baselineSchema.parse(body);

    const service = new ProjectionService(session.user.id);
    const baseline = await service.calculateBaseline(timeHorizon);
    ...
    console.error('Erreur lors du calcul de la baseline:', error);
    return NextResponse.json(
